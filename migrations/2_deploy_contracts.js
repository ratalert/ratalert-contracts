...
module.exports = async (deployer, network, accounts) => {
  ...
  const character = await deployProxy(Character, [paywall.address, mint.address, traits.address], {deployer});
  ...
  await character.configure(...config.character, properties.address);
  ...
  await mcStake.configure(config.kitchen.mcStake.foodTokenMaxSupply, [config.kitchen.mcStake.dailyChefEarnings, config.kitchen.ratTheftPercentage, config.kitchen.vestingPeriod, config.kitchen.accrualPeriod], config.kitchen.mcStake.propertyIncrements, config.kitchen.chefEfficiencyMultiplier, config.kitchen.ratEfficiencyMultiplier, config.kitchen.ratEfficiencyOffset, config.kitchen.maxClaimsPerTx, config.kitchen.claimFee);
