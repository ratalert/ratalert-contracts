...
contract('Paywall (proxy)', (accounts) => {
  ...
  describe('toggleWhitelist()', () => {
    it('turns it on', async () => {
      ...
      await scheduleAndExecute(this.paywall, 'toggleWhitelist', [true], { from: dao }, salt++);
      await expect(this.paywall.onlyWhitelist()).to.eventually.be.true;
    });

    it('turns it off', async () => {
      await scheduleAndExecute(this.paywall, 'toggleWhitelist', [false], { from: dao });
      await expect(this.paywall.onlyWhitelist()).to.eventually.be.false;
    });
  });
  ...
  describe('addToWhitelist()', () => {
    it('adds addresses', async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        let res = await scheduleAndExecute(this.paywall, 'addToWhitelist', [[owner, anon]], { from: dao }, salt++);
  ...
  describe('removeFromWhitelist()', () => {
    it('removes addresses', async () => {
      for (let i = 4; i >= 0; i--) {
        const res = await scheduleAndExecute(this.paywall, 'removeFromWhitelist', [[owner, anon]], { from: dao }, salt++);
        const logs = decodeRawLogs(res, this.paywall, 'UpdateWhitelist');
        expect(logs).to.have.length(2);
        expect(logs[0].event).to.equal('UpdateWhitelist');
        expect(logs[1].event).to.equal('UpdateWhitelist');
    ...
    it('does not overflow', async () => {
      const res = await scheduleAndExecute(this.paywall, 'removeFromWhitelist', [[owner]], { from: dao }, salt++);
      const logs = decodeRawLogs(res, this.paywall, 'UpdateWhitelist');
  ...
  describe('addToFreeMints()', () => {
    it('adds addresses', async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        const res = await scheduleAndExecute(this.paywall, 'addToFreeMints', [[owner, anon]], { from: dao }, salt++);
  ...
  describe('removeFromFreeMints()', () => {
    it('removes addresses', async () => {
      for (let i = 4; i >= 0; i--) {
        const res = await scheduleAndExecute(this.paywall, 'removeFromFreeMints', [[owner, anon]], { from: dao }, salt++);
        const logs = decodeRawLogs(res, this.paywall, 'UpdateFreeMints');
        expect(logs).to.have.length(2);
        expect(logs[0].event).to.equal('UpdateFreeMints');
        expect(logs[1].event).to.equal('UpdateFreeMints');
    ...
    it('does not overflow', async () => {
      const res = await scheduleAndExecute(this.paywall, 'removeFromFreeMints', [[owner]], { from: dao }, salt++);
      const logs = decodeRawLogs(res, this.paywall, 'UpdateFreeMints');
      expect(logs).to.have.length(1);
      expect(logs[0].event).to.equal('UpdateFreeMints');
      expect(logs[0].args.amount).to.be.a.bignumber.eq('0');
  ...
  describe('handle()', () => {
    it('fails without sufficient whitelist spots', async () => {
      await scheduleAndExecute(this.paywall, 'toggleWhitelist', [true], { from: dao }, salt++);
      await scheduleAndExecute(this.paywall, 'addToWhitelist', [[owner, owner]], { from: dao }, salt++);
    ...
    it('succeeds with sufficient whitelist spots', async () => {
      await scheduleAndExecute(this.paywall, 'addToWhitelist', [[owner]], { from: dao }, salt++);
    ...
    it('succeeds with whitelisting disabled', async () => {
      await scheduleAndExecute(this.paywall, 'toggleWhitelist', [false], { from: dao }, salt++);
      const { logs } = await this.paywall.handle(anon, 2, toWei(0.2), 0, 100, 10);
      expect(logs).to.have.length(0);
    });
    it('still gets a discount', async () => {
      await scheduleAndExecute(this.paywall, 'addToWhitelist', [[anon]], { from: dao }, salt++);
    ...
    it('succeeds with sufficient free mint spots', async () => {
      await scheduleAndExecute(this.paywall, 'toggleWhitelist', [true], { from: dao }, salt++);
      await scheduleAndExecute(this.paywall, 'addToFreeMints', [[anon, anon, anon]], { from: dao }, salt++);
    ...
    it('uses free mints before whitelist spots', async () => {
      await scheduleAndExecute(this.paywall, 'addToFreeMints', [[anon]], { from: dao }, salt++);
      await scheduleAndExecute(this.paywall, 'addToWhitelist', [[anon]], { from: dao }, salt++);
