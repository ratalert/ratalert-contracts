...
contract('Paywall (proxy)', (accounts) => {
  ...
  before(async () => {
    ...
    await this.paywall.addController([owner], { from: dao });
  });
  ...
  describe('addToWhitelist()', () => {
    it('adds addresses', async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        const { logs } = await this.paywall.addToWhitelist([owner, anon], { from: dao });
  ...
  describe('removeFromWhitelist()', () => {
    it('removes addresses', async () => {
      for (let i = 4; i >= 0; i--) {
        const { logs } = await this.paywall.removeFromWhitelist([owner, anon], { from: dao });
        expect(logs).to.have.length(2);
        expect(logs[0].event).to.equal('UpdateWhitelist');
        expect(logs[1].event).to.equal('UpdateWhitelist');
        expect(logs[0].args.amount).to.be.a.bignumber.eq(i.toString());
    ...
    it('does not overflow', async () => {
      const { logs } = await this.paywall.removeFromWhitelist([owner], { from: dao });
  ...
  describe('addToFreeMints()', () => {
    it('adds addresses', async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        const { logs } = await this.paywall.addToFreeMints([owner, anon], { from: dao });
        expect(logs).to.have.length(2);
  ...
  describe('removeFromFreeMints()', () => {
    it('removes addresses', async () => {
      for (let i = 4; i >= 0; i--) {
        const { logs } = await this.paywall.removeFromFreeMints([owner, anon], { from: dao });
        expect(logs).to.have.length(2);
        expect(logs[0].event).to.equal('UpdateFreeMints');
        expect(logs[1].event).to.equal('UpdateFreeMints');
        expect(logs[0].args.amount).to.be.a.bignumber.eq(i.toString());
    ...
    it('does not overflow', async () => {
      const { logs } = await this.paywall.removeFromFreeMints([owner], { from: dao });
      expect(logs).to.have.length(1);
      expect(logs[0].event).to.equal('UpdateFreeMints');
      expect(logs[0].args.amount).to.be.a.bignumber.eq('0');
      await expect(this.paywall.freeMints(owner)).to.eventually.be.a.bignumber.eq('0');
